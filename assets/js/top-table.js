...
if(!container)return;
const safeColor=v=>typeof v==='string'&&/^#[0-9a-f]{6}$/i.test(v)?v:'#eef3f8';
const safeHref=v=>{try{const u=new URL(String(v||''),location.href);return /^https?:$/.test(u.protocol)?u.href:'#';}catch{return '#';}};
...
const render=rows=>{container.replaceChildren();rows.forEach((item,index)=>{
  ...
  const brandWrap=make('div','brand-wrap');brandWrap.appendChild(make('div','brand',String(item.brand||'')));
  ...
  const cta=make('a','cta','Jouer');cta.href=safeHref(item.cta_url);cta.target='_blank';cta.rel='nofollow sponsored noopener';row.appendChild(cta);
  container.appendChild(row);
});};
fetch(DATA_URL,{method:'GET',mode:'cors',credentials:'omit',cache:'no-store'})
